...
public class Bauen extends MatPoolScript implements Cloneable{
	...
	private boolean foundNeededBuilding=false;
...
public void runScript(int scriptDurchlauf){
	...
	private void BauManager(){
		...
		if (this.spec==Bauen.STRASSE){
			if (this.scriptUnit.getSkillLevel("Straßenbau")<this.minStassenbauTalent){
				this.addComment("Bauen: minTalent Straßenbau nicht erreicht, ich sollte lernen.");
				this.lernTalent="Straßenbau";
				this.setFinalStatusInfo("Min Straßenbau. ");
				this.setAutomode_hasPlan(true);
				isLearning=true;
				this.LernfixOrder="Talent=Straßenbau";
				this.Lerne();
			} else {
				this.addComment("Bauen: minTalent Straßenbau ist erreicht, ich suche Arbeit.");
			}
		} else {
	...
	public void vorMatPool(){
		...
		if (this.actTyp== Bauen.STRASSE){
			...
			if (neededBuildingName=="") {
				...
				if (foundNeededBuilding) {
					// auf Unterhalt prüfen, zum Strassenbau muss das Gebäude unterhalt haben
					if (FFToolsRegions.hasSupportedRegionSpecificBuilding(this.scriptUnit.getUnit().getRegion(), neededBuildingName)) {
						this.addComment("Keine Infos über Unterhaltsproblem bekannt beim notwendigen Gebäude: " + neededBuildingName);
					} else {
						this.doNotConfirmOrders("!!!Bauen: Unterhaltsproblem bekannt beim notwendigen Gebäude: " + neededBuildingName);
						foundNeededBuilding=false;
					}
				}
	...
	private void vorMP_Strasse(){
		...
		if (this.actSize<this.targetSize){
			...
			this.addComment(s);
			statusInfo+=s;
			this.steinRequest = new MatPoolRequest(this,(this.targetSize - this.actSize),"Stein",this.prioSteine,"Straßenbau " + this.dirLocal);
	...
	private void nachMP_Strasse(){
		...
		// Step 2 was könnten wir maximal nach Talentstand der Einheit bauen?
		SkillType bauType = this.gd_Script.getRules().getSkillType("Straßenbau",false);
		int anzTalPoints = 0;
		int Peff = 0; // effektive Personenanzahl
		Peff = FFToolsUnits.getPersonenEffektiv(this.scriptUnit);
		if (bauType!=null){
		...
		this.addComment("Bauen: Einheit ist fähig für " + anzTal + " Stufen bei der Straße");
		...
		boolean okAusl = false;
		int Auslastung = (int)Math.floor(((double)actAnz/(double)anzTal) * 100);
		this.addComment("Bauen: Auslastung bei Straßenbau: " + Auslastung + "% (min:" + this.minAuslastung + "%)");
		if (Auslastung>=this.minAuslastung){
			okAusl=true;
		}
		...
		if (okAusl || complete){
			...
			this.setFinalStatusInfo("baut Straße");
			checkUnterhaltGebäude();
		} else {
			// nicht bauen
			this.addComment("Bauen: Straße (" + this.dirLocal + ") wird diese Runde nicht weitergebaut.");
			this.setBauBefehl("","nach MP Straße");
	...
	private void checkUnterhaltGebäude() {
		...
		if (neededBuildingName!="" && preRequisteBuilding!=null) {
			if (!FFToolsRegions.sindMauernEwig(preRequisteBuilding)) {
				...
				this.addComment("Bauen: notwendiges Gebäude ist magisch vom Unterhalt befreit (" + neededBuildingName + ")");
			}
		}
	}
