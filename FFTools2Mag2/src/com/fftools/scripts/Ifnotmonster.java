...
public class Ifnotmonster extends Script{
	...
	public void runScript(int scriptDurchlauf){
		...
		if (super.getArgCount()<1) {
			...
			super.scriptUnit.doNotConfirmOrders("Falscher Aufruf von IfNotMonster: zu geringe Anzahl Parameter.");
			super.addComment("Unit wurde durch IfNotMonster NICHT bestaetigt", true);
			addOutLine("X....Falscher Aufruf von IfNotMonster: zu geringe Anzahl Parameter: " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
		}
		...
		if (super.getArgCount()>0) {
			...
			if (keyWord.equalsIgnoreCase("includingFactionDisguised")) {
				...
				this.addComment("ifNotMonster: auch parteigetarnte Einheiten, deren Fraktion uns unbekannt ist, gelten als Monster... (includingFactionDisguised)");
				argStartCount=2;
			}
			...
			if (enemyUnit==""){
				this.addComment("IfNotMonster: No Monster detected: ");
				if (keyWord.equalsIgnoreCase("script")) {
					if (super.getArgCount()>argStartCount) {
						...
						addOutLine("X....Unerwartetes Ende der Befehlszeile (IfNotMonster,script): " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
					}
				} else {
					...
					if (newOrderLine.length()>0){
						...
						newOrderLine = newOrderLine.concat(" ;script ifNotMonster");
						super.addOrder(newOrderLine,true);
						super.addComment("Unit wurde durch IfNotMonster befehligt", true);
					}
				}
			} else {
				this.addComment("IfNotMonster: presence of monsters found in this region: " + enemyUnit);
			}
		}
	}
	/**
