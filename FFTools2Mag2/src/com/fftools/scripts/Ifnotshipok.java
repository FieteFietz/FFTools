...
public class Ifnotshipok extends Script{
	...
	public void runScript(int scriptDurchlauf){
		...
		if (s==null){
			...
			this.addComment("!!! IfNotShipOK macht keinen Sinn, wenn ich nicht an Bord bin!!!");
			return;
		}
		...
		if (u==null){
			this.doNotConfirmOrders();
			this.addComment("!!! IfNotShipOK macht keinen Sinn, den das Schiff hat keinen Kapitän!!!");
			return;
		}
		
		if (!u.equals(this.scriptUnit.getUnit())){
			this.doNotConfirmOrders();
			this.addComment("!!! IfNotShipOK macht keinen Sinn, wenn ich nicht der Kapitän bin!!!");
			return;
		}
		...
		if (shipNotOK){
			...
			if (keyWord.equalsIgnoreCase("script")) {
				if (super.getArgCount()>1) {
					...
					super.addComment("Unit wurde durch IfNotShipOK NICHT bestaetigt", true);
