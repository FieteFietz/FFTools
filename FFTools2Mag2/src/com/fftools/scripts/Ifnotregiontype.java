...
public class Ifnotregiontype extends Script{
	...
	public void runScript(int scriptDurchlauf){
		...
		if (super.getArgCount()<2) {
			...
			super.scriptUnit.doNotConfirmOrders("Falscher Aufruf von IfNotRegionType: zu geringe Anzahl Parameter.");
			parseOK=false;
			addOutLine("X....Falscher Aufruf von IfNotRegionType: zu geringe Anzahl Parameter: " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
		}
		...
		if (super.getArgCount()>1) {
			...
			if (regionWanted == null || regionWanted.length()==0) {
				super.scriptUnit.doNotConfirmOrders("Fehler beim Erkennen des RegionsTypes (IfNotRegionType");
				parseOK=false;
			...
			if (parseOK) {
				if (this.getUnit().getRegion().getRegionType().getName().equalsIgnoreCase(regionWanted)) {
					this.addComment("IfNotRegionType: " + regionWanted + " als vorhanden erkannt");
				} else {
					rightRegionType=false;
					this.addComment("IfNotRegionType: " + regionWanted + " als NICHT vorhanden erkannt");
				}
			}
			...
			if (!rightRegionType){
				...
				if (keyWord.equalsIgnoreCase("script")) {
					if (super.getArgCount()>2) {
						...
						super.scriptUnit.doNotConfirmOrders("Unerwartetes Ende der Befehlszeile (script IfNotRegionType)");
						addOutLine("X....Unerwartetes Ende der Befehlszeile (IfNotRegionType,script): " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
					}
				} else {
					...
					if (newOrderLine.length()>0){
						...
						newOrderLine = newOrderLine.concat(" ;script ifNotRegionType");
						super.addOrder(newOrderLine,true);
						super.addComment("Unit wurde durch IfNotRegionType bestaetigt", true);
					}
				}
			}
		}
