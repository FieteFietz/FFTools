...
public class Ifnotunit extends Script{
	...
	public void runScript(int scriptDurchlauf){
		...
		if (super.getArgCount()<2) {
			...
			super.scriptUnit.doNotConfirmOrders("Falscher Aufruf von IfNotUnit: zu geringe Anzahl Parameter.");
			
			addOutLine("X....Falscher Aufruf von IfNotUnit: zu geringe Anzahl Parameter: " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
		} else {
			// gibts die unit?
			...
			if (checkU.getName().equalsIgnoreCase(unitDesc) || checkU.toString(false).equalsIgnoreCase(unitDesc)){
				// soso, ein Witzbold..
				super.scriptUnit.doNotConfirmOrders("Ich bin ich, meiner selbst, und mir - ganz ich. (IfNotUnit");
				addOutLine("X....Falscher Aufruf von IfNotUnit: auf sich selbst angewendet: " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
			} else {
			...
			// Ergebnis ?
			if (!found){
				super.addComment("IfNotUnit: Unit " + unitDesc + " wurde nicht gefunden.", true);
				// nicht gefunden...und damit Bedingung erfüllt
				...
				if (keyWord.equalsIgnoreCase("script")) {
					if (super.getArgCount()>2) {
						...
						addOutLine("X....Unerwartetes Ende der Befehlszeile (IfNotUnit, script): " + this.scriptUnit.getUnit().toString(true) + " in " + this.scriptUnit.getUnit().getRegion().toString());
					}
				} else {
				...
				super.addComment("IfNotUnit: Unit " + unitDesc + " wurde gefunden.", true);
			}
		}
	}
