 ...
 * @version
 */
public class TeilenJeAnzahlAction extends MenuAction {
	 ...
	 */
	public TeilenJeAnzahlAction(SelectionObserver selectionObserver, int Anzahl) {
        super(selectionObserver.getClient());
        this.setName("Teilen in je " + Anzahl + " Pers");
	...
	private void doAction(Unit u){
		...
		int personsPerTemp = this.Anzahl;
		int countTempUnits = (int)Math.round((double)allPersons/(double)this.Anzahl);
		...
		for (int i = 1;i<=countTempUnits;i++){
