 ...
 * @version
 */
public class MatrosenKaravelleAction extends MenuAction {
	 ...
	 */
	public MatrosenKaravelleAction(SelectionObserver selectionObserver) {
        super(selectionObserver.getClient());
        setName("Temp: Matrosen Kara");
	...
	/**
	 * führt den Rekrutierevorgang für diese Unit durch
	 * @param u
	 */
	private void makeIt(Unit u){
		...
		// name setzen
		tempUnit.addOrder("BENENNEN EINHEIT Matrosen ;dnt");
		// script Setzen
		tempUnit.addOrder("// script Lernfix Talent=Segeln Ziel=5");
		
		// rekrutieren
		tempUnit.addOrder("// script Runde " + this.selectionObserver.getClient().getData().getDate().getDate() + " script Rekrutieren 6");
		tempUnit.addOrder("Rekrutiere 6");
