 ...
 * @version
 */
public class MatrosenTriremeAction extends MenuAction {
	 ...
	 */
	public MatrosenTriremeAction(SelectionObserver selectionObserver) {
        super(selectionObserver.getClient());
        setName("Temp: Matrosen Tri");
	...
	private void makeIt(Unit u){
		...
		// script Setzen
		tempUnit.addOrder("// script Lernfix Talent=Segeln Ziel=6");
		
		// rekrutieren
		tempUnit.addOrder("// script Runde " + this.selectionObserver.getClient().getData().getDate().getDate() + " script Rekrutieren 20");
		tempUnit.addOrder("Rekrutiere 20");
